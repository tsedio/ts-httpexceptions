...
class DocSymbol {
  ...
  isPrivate() {
    if (this.private === undefined) {
      if (this.symbolType !== 'interface') {
        ...
        const modulePath = this.module.name.replace(name, '../../lib/')
        const path = require('path').resolve(modulePath.replace('index.ts', ''))
  ...
  modulePath() {
    ...
    return this.srcPath.replace(info.root + '/src', name + '/lib').replace(/\.ts$/, '')
  }
